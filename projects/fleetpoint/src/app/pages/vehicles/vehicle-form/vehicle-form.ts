...
import { DeviceOption, InventoryOption, VehicleInventoryApiService, VehicleInventoryRecord } from '../../../shared/services/vehicle-inventory-api.service';
...
export class VehicleForm implements OnChanges {
  ...
  constructor(private readonly formBuilder: FormBuilder, private readonly api: VehicleInventoryApiService, private readonly feedback: FeedbackDialogBridgeService) {
    ...
    this.form = this.formBuilder.nonNullable.group({
      ...
      status: [true], is_immobilization_enabled: [true],
    });
  }

  ngOnChanges(changes: SimpleChanges): void { if (changes['open']?.currentValue) { this.prepareForm(); this.loadOptions(); } }
  ...
  protected submit(): void {
    ...
    const values: Record<string, string | number | boolean> = {
      ...raw, name: raw.registration, fleet_category: raw.category, status: raw.status ? 1 : 2,
    ...
    const request = this.editing() ? this.api.updateVehicle(this.vehicle()!.id, payload) : this.api.createVehicle(payload);
    request.pipe(finalize(() => this.loading.set(false))).subscribe({
      next: async () => {
        const action = this.editing() ? 'updated' : 'added';
        await this.feedback.open({ type: 'success', title: `Vehicle ${action}`, message: `${raw.registration} was ${action} successfully.`, confirmText: 'Done', showCancel: false });
        this.created.emit(raw as VehicleFormValue);
        this.reset();
      },
      error: (response) => { const message = response.error?.message || `Vehicle could not be ${this.editing() ? 'updated' : 'created'}.`; this.error.set(message); void this.feedback.open({ type: 'error', title: `Unable to ${this.editing() ? 'update' : 'add'} vehicle`, message, confirmText: 'Close', showCancel: false }); },
    });
  }
    ...
    forkJoin({ fleets: this.api.getFleetOptions(), categories: this.api.getCategoryOptions(), types: this.api.getVehicleTypeOptions(), devices: this.api.getAvailableDevices() })
      .subscribe({
        next: ({ fleets, categories, types, devices }) => {
          this.fleets.set(fleets.data?.data ?? []); this.categories.set(categories.data?.data ?? []); this.vehicleTypes.set(types.data?.data ?? []);
          const available = devices.data?.data ?? []; const vehicle = this.vehicle();
          this.devices.set(vehicle?.device && !available.some((device) => String(device.id) === String(vehicle.device)) ? [{ id: Number(vehicle.device), device_id: vehicle.device_id || String(vehicle.device) }, ...available] : available);
        },
  ...
  private reset(): void { this.form.reset({ year: new Date().getFullYear(), odo_reading: 0, engine_capacity: 0, wheels: 4, fuel_tank_capacity: 0, purchase_type: '2', speed: false, speed_threshold: 0, harsh_acceleration: false, harsh_braking: false, sharp_turning: false, status: true, is_immobilization_enabled: true }); this.activeStep.set(0); this.submitted.set(false); this.error.set(''); this.imageFile.set(null); this.imagePreview.set('assets/fleetpoint/vehicle.svg'); }

  private prepareForm(): void {
    this.reset(); const vehicle = this.vehicle(); if (!vehicle) return;
    const threshold = Number(vehicle.speed_threshold || 0);
    this.form.patchValue({
      registration: vehicle.registration || vehicle.name, engine_number: vehicle.engine_number, chassis_number: vehicle.chassis_number, make: vehicle.make, model: vehicle.model,
      year: Number(vehicle.year), color: vehicle.color, odo_reading: Number(vehicle.odo_reading || 0), engine_capacity: Number(vehicle.engine_capacity || 0), wheels: Number(vehicle.wheels || 4),
      fuel_tank_capacity: Number(vehicle.fuel_tank_capacity || 0), purchase_type: String(vehicle.purchase_type || '2'), type: String(vehicle.type || ''), device: String(vehicle.device || ''),
      date_commissioned: vehicle.date_commissioned?.slice(0, 10) || '', owner: vehicle.owner || '', owner_id: vehicle.owner_id, nationality: vehicle.nationality,
      registration_date: vehicle.registration_date?.slice(0, 10) || '', expiry_date: vehicle.expiry_date?.slice(0, 10) || '', fleet: String(vehicle.fleet ?? ''), category: String(vehicle.fleet_category ?? ''),
      speed: threshold > 0, speed_threshold: threshold, harsh_acceleration: !!vehicle.harsh_acceleration, harsh_braking: !!vehicle.harsh_braking, sharp_turning: !!vehicle.sharp_turning,
      status: vehicle.status === 1, is_immobilization_enabled: vehicle.is_immobilization_enabled !== false,
    });
