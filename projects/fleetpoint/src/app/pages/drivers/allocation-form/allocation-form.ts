...
import { DriverApiService, DriverVehicleAllocation } from '../../../shared/services/driver-api.service';
...
export class AllocationForm implements OnChanges {
  ...
  protected submit():void{
    this.submitted.set(true);this.form.markAllAsTouched();if(this.form.invalid||!this.form.controls.drivers.value.length)return;
    const value=this.form.getRawValue();if(value.end_date<value.start_date){this.error.set('End date must be on or after the start date.');return;}
    this.loading.set(true);this.error.set('');
    const payload={vehicle:Number(value.vehicle),driver:value.drivers.map(Number),start_date:value.start_date,end_date:value.end_date};const current=this.allocation();
    const request=current?this.api.updateDriverVehicleAllocation(current.id,current.vehicle_id,String(current.vehicle_id)!==value.vehicle,payload):this.api.createDriverVehicleAllocation(payload);
    request.pipe(finalize(()=>this.loading.set(false))).subscribe({
      next:async response=>{const action=current?'updated':'added';await this.feedback.open({type:'success',title:`Allocation ${action}`,message:response.message||`The driver allocation was ${action} successfully.`,confirmText:'Done',showCancel:false});this.saved.emit();this.reset();},
      error:response=>{const message=response.error?.message||'The allocation could not be saved.';this.error.set(message);void this.feedback.open({type:'error',title:'Unable to save allocation',message,confirmText:'Close',showCancel:false});}
    });
  }
