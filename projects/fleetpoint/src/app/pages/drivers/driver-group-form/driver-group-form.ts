...
export class DriverGroupForm implements OnChanges {
      ...
      .subscribe({
        next: (response) => {
          ...
          const active = (response.data?.data ?? []).filter(
            (driver) => driver.status === 1 || driver.status === '1',
          );
          const available = [
            ...current,
            ...active.filter((driver) => !current.some((item) => item.id === driver.id)),
          ];
        ...
        error: (response) =>
          this.error.set(response.error?.message || 'Drivers could not be loaded.'),
      });
  }
  protected selectionChanged(ids: string[]): void {
    this.form.controls.drivers_list.setValue(ids);
    this.form.controls.drivers_list.markAsTouched();
  }
  ...
  protected submit(): void {
    ...
    const request = this.editing()
      ? this.api.updateDriverGroup(this.group()!.id, payload)
      : this.api.createDriverGroup(payload);
    request.pipe(finalize(() => this.loading.set(false))).subscribe({
      next: async () => {
        const action = this.editing() ? 'updated' : 'added';
        await this.feedback.open({
          type: 'success',
          title: `Driver group ${action}`,
          message: `${raw.name.trim()} was ${action} successfully.`,
          confirmText: 'Done',
          showCancel: false,
        });
        this.created.emit();
      ...
      error: (response) => {
        ...
        void this.feedback.open({
          type: 'error',
          title: 'Unable to add driver group',
          message,
          confirmText: 'Close',
          showCancel: false,
        });
  ...
  private reset(): void {
    this.form.reset({ name: '', drivers_list: [] });
  ...
  private patchGroup(): void {
    ...
    this.form.patchValue({ name: group.name, drivers_list: driverIds.map(String) });
  }
}
