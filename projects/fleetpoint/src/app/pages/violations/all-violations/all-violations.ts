...
@Component({selector:'app-all-violations',imports:[DataTable,Dropdown],templateUrl:'./all-violations.html',styleUrl:'./all-violations.css'})
export class AllViolations {
  ...
  protected readonly categoryOptions=CATEGORY_OPTIONS;protected readonly severityOptions=SEVERITY_OPTIONS;protected readonly sourceOptions=SOURCE_OPTIONS;protected readonly actions:TableAction[]=['view','edit'];
  protected readonly columns:TableColumn[]=[{key:'type',label:'Violation',type:'user',secondaryKey:'id'},{key:'driver',label:'Driver'},{key:'vehicle',label:'Vehicle',clickable:true},{key:'category',label:'Category',type:'status'},{key:'severity',label:'Severity',type:'priority'},{key:'source',label:'Source',type:'status'},{key:'location',label:'Location'},{key:'timestamp',label:'Date & Time'},{key:'review',label:'Review',type:'status'},{key:'actions',label:'Actions',type:'actions'}];
