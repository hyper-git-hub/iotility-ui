...
export interface DriverViolationMetric {
  label: string;
  value: number;
}
export interface DriverIdlePoint {
  date: string;
  value: number;
}
...
export class DriverDetailApiService {
  private readonly driverApi = `${environment.driverBaseUrl}/driver`;
  ...
  getDriver(id: number): Observable<ApiResponse<{ count: number; data: DriverRecord[] }>> {
    return this.http.get<ApiResponse<{ count: number; data: DriverRecord[] }>>(
      `${this.driverApi}/`,
      {
        params: { id: String(id) },
      },
  ...
  getGraphs(id: number, start: string, end: string): Observable<ApiResponse<DriverDetailGraphs>> {
    ...
    return this.http.get<ApiResponse<DriverDetailGraphs>>(
      `${this.driverApi}/driver-details-graphs`,
      { params },
    );
  }
  ...
  getViolations(
    ...
    limit: number,
    offset: number,
  ): Observable<ApiResponse<{ count: number; data: DriverViolationRecord[] }>> {
    const params = new HttpParams()
      .set('limit', limit)
      .set('offset', offset)
      .set('start_datetime', start)
    ...
    return this.http.get<ApiResponse<{ count: number; data: DriverViolationRecord[] }>>(
      `${this.fleetApi}/common/violation`,
      { params },
    );
  }
}
